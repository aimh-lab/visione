...
function submitResultAVS(id, queries) {
	return $.ajax({
		type : "GET",
		async : true,
		url : urlBSService+"/submitResult?id="+ id + "&isAVS=true&query=" + encodeURIComponent(queries[0]) + "&query2=" + encodeURIComponent(queries[1]) + "&simreorder=" + simreorder,
	}).responseText;
}


function submitResult(id, query) {
	return $.ajax({
		type : "GET",
		async : false,
		url : urlBSService+"/submitResult?id="+ id+ "&query=" + query,
	}).responseText;
}
		...
		for (i = 0; i < res.length; i++) {
			...
			for (splitIdx = 0; splitIdx < resSplit.length; splitIdx++) {
				...
				imgtable += '<td>'
					...
					+'<img style="padding: 3px;" src="img/gem_icon.svg" width=22 title="image similarity" alt="' + visioneID + '" id="gemSim' + idHref + '" onclick="var queryObj=new Object(); queryObj.vf=\'' + visioneID + '\'; searchByLink(queryObj); return false;">'
					+'<img style="padding: 3px;" src="img/tern_icon.svg" width=22 title="semantic similarity" alt="' + visioneID + '" id="ternSim' + idHref + '" onclick="var queryObj=new Object(); queryObj.ternSim=\'' + visioneID + '\'; searchByLink(queryObj); return false;">'
					+'<img style="padding: 3px;" src="img/clip_icon.svg" width=22 title="semantic video similarity" alt="' + visioneID + '" id="clipSim' + '" onclick="var queryObj=new Object(); queryObj.clipSim=\'' + visioneID + '\'; searchByLink(queryObj); return false;">'
					...
					+'<span class="pull-right"><i title="Submit result" class="fa fa-arrow-alt-circle-up" style="font-size:17px; color:#00AA00; padding-left: 0px;" onclick="submitWithAlert(\''+ visioneID+ '\',\''+submitQuery+'\'); return false;"></i></span>'
			...
			resSplit = imgId.split("__");
				...
				function hoverVideo(e) {
					$('#' + this.id).contextmenu(function() {
						...
						imgtable = '<video style="' + backgroundImg + '" id="' + playerId + '" class="myimg video" autoplay loop muted preload="none"><source src="' + videourl + '#t=' + startTime + ',' + endTime + '" type="video/mp4"></video>'
