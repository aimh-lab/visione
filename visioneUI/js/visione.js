		...
		if (res.length == 0)
			...
			for (let i = 0; i < res.length; i++) {
				...
				if (videoId != prevID) {
					...
					imgGridResults += '<div data-videoid="' + videoId + '" class="item column-span-1"><a href="showVideoKeyframes.html?collection=' + collection + '&videoId=' + videoId + '&id=' + imgId + '" target="_blank">' + videoId + '<a></div>';
				...
				avsObj = getAvsObj(collection, videoId, imgId, 'avs_' + imgId, thumbnailUrl + path, keyFramesUrl + path, resrowIdx, resColIdx - 1)
...
const imgResult = (res, borderColor, avsObj, isSimplified = false) => {
	...
	if (isSimplified) {
		return `
			...
			<a class="font-tiny" title="View annotations of ${res.frameName},  Score: ${res.score}" href="indexedData.html?collection=${res.collection}&videoId=${res.videoId}&id=${res.imgId}" target="_blank"> ${res.frameNumber}</a>
			<a title="Video summary" href="showVideoKeyframes.html?collection=${res.collection}&videoId=${res.videoId}&id=${res.imgId}#${res.frameName}" target="_blank"><i class="fa fa-th font-normal" style="padding-left: 3px;"></i></a>
	...
	return `
		...
		<span class="pull-right"><i title="Submit result" class="fa fa-arrow-alt-circle-up font-huge" style="color:#00AA00; padding-left: 0px;" onclick="submitWithAlert('${res.imgId}','${res.videoId}','${res.collection}'); return false;"></i></span>'
		<div class="myimg-thumbnail" style="border-color:${borderColor};" id="${res.imgId}" lang="${res.collection}collection|${res.videoId}|${res.videoId}|${res.videoUrlPreview}" onclick='avsCleanManuallySelected(); avsToggle(${avsObj}, event)'>
